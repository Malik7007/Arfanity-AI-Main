<script>
	...
	import { AI_API_BASE_URL, AI_BASE_URL, AI_HOSTNAME } from '$lib/constants';
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${AI_BASE_URL}` || undefined, {
			reconnection: true,
		...
		_socket.on('connect', async () => {
			...
			if (version !== null || deploymentId !== null) {
				if (
					($AI_VERSION !== null && version !== $AI_VERSION) ||
					($AI_DEPLOYMENT_ID !== null && deploymentId !== $AI_DEPLOYMENT_ID)
				) {
					await unregisterServiceWorkers();
			...
			if (deploymentId !== null) {
				AI_DEPLOYMENT_ID.set(deploymentId);
			}

			if (version !== null) {
				AI_VERSION.set(version);
				window.AI_VERSION = version;
			}
		...
		if (type === 'calendar:alert' && data) {
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • Arfanity AI`, {
						body: timeStr,
						icon: `${AI_BASE_URL}/static/favicon.png`
					});
				}
			}
			return;
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isInBackground) {
			if (type === 'chat:completion') {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • Arfanity AI`, {
								body: content,
								icon: `${AI_BASE_URL}/static/favicon.png`
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • Arfanity AI`, {
							body: data?.content,
							icon: `${AI_API_BASE_URL}/users/${data?.user?.id}/profile/image`
						});
					}
				}
