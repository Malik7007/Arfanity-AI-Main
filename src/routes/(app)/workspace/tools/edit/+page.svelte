<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_arfanity_ai_version ?? '0.0.0', AI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'Arfanity AI version (v{{arfanity_ai_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						arfanity_ai_VERSION: AI_VERSION,
						REQUIRED_VERSION: manifest?.required_arfanity_ai_version ?? '0.0.0'
					}
				)
			);
			return;
