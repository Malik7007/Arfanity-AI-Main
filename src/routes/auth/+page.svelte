<script lang="ts">
	...
	import { AI_NAME, config, user, socket } from '$lib/stores';
	...
	const setSessionUser = async (sessionUser, redirectPath: string | null = null) => {
		if (sessionUser) {
			...
			// Keep a last-known-good session snapshot for resilient refresh restore.
			localStorage.setItem('session-user-cache', JSON.stringify(sessionUser));
			...
			<div class="w-full px-10 min-h-screen flex flex-col text-center">
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
					<div class=" my-auto pb-10 w-full sm:max-w-md">
						...
						>
							<div>
								{$i18n.t('Signing in to {{AI_NAME}}', { AI_NAME: $AI_NAME })}
							</div>
								...
								<div class="mb-1">
									<div class=" text-2xl font-medium">
										{#if $config?.onboarding ?? false}
											{$i18n.t(`Get started with {{AI_NAME}}`, { AI_NAME: $AI_NAME })}
										{:else if mode === 'ldap'}
											{$i18n.t(`Sign in to {{AI_NAME}} with LDAP`, { AI_NAME: $AI_NAME })}
										{:else if mode === 'signin'}
											{$i18n.t(`Sign in to {{AI_NAME}}`, { AI_NAME: $AI_NAME })}
										{:else}
											{$i18n.t(`Sign up to {{AI_NAME}}`, { AI_NAME: $AI_NAME })}
										{/if}
									</div>
								...
								<div class="flex flex-col space-y-2">
									{#if $config?.oauth?.providers?.google}
										<button
											...
											on:click={() => {
												window.location.href = `${AI_BASE_URL}/oauth/google/login`;
											}}
										>
											<svg
											...
											>
												<path
													...
													fill="#34A853"
													d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"></path><path fill="none" d="M0 0h48v48H0z"></path>
											</svg>
									...
									{#if $config?.oauth?.providers?.microsoft}
										<button
											class="flex justify-center items-center bg-gray-700/5 hover:bg-gray-700/10 dark:bg-gray-100/5 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											on:click={() => {
												window.location.href = `${AI_BASE_URL}/oauth/microsoft/login`;
											}}
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
									...
									{#if $config?.oauth?.providers?.github}
										<button
											class="flex justify-center items-center bg-gray-700/5 hover:bg-gray-700/10 dark:bg-gray-100/5 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											on:click={() => {
												window.location.href = `${AI_BASE_URL}/oauth/github/login`;
											}}
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
									...
									{#if $config?.oauth?.providers?.oidc}
										<button
											class="flex justify-center items-center bg-gray-700/5 hover:bg-gray-700/10 dark:bg-gray-100/5 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											on:click={() => {
												window.location.href = `${AI_BASE_URL}/oauth/oidc/login`;
											}}
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
									...
									{#if $config?.oauth?.providers?.feishu}
										<button
											class="flex justify-center items-center bg-gray-700/5 hover:bg-gray-700/10 dark:bg-gray-100/5 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											on:click={() => {
												window.location.href = `${AI_BASE_URL}/oauth/feishu/login`;
											}}
										>
