<script>
	...
	import { AI_NAME, config } from '$lib/stores';
			...
			<div class="m-auto pb-44 flex flex-col justify-center">
				<div class="max-w-md">
					<div class="text-center text-2xl font-medium z-50">
						{$i18n.t('{{AIName}} Backend Required', { AIName: $AI_NAME })}
					</div>

					<div class=" mt-4 text-center text-sm w-full">
						{$i18n.t(
							"Oops! You're using an unsupported method (frontend only). Please serve the AI from the backend."
						)}
						...
						>
						{$i18n.t('or')}
						<a  class=" font-medium underline" href="#" target="_blank" rel="noopener noreferrer" 
							>{$i18n.t('join our community for help.')}</a
						>
					</div>
