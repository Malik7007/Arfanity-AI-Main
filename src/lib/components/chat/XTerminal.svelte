<script lang="ts">
	...
	const getTerminalInfo = (): { serverId: string; baseUrl: string } | null => {
		...
		if (systemMatch) {
			// For system terminals, WS goes through the Arfanity AI backend proxy
			return { serverId: systemMatch.id, baseUrl: AI_API_BASE_URL };
		}
	...
	const connect = async () => {
		...
		try {
			...
			if (info.serverId === '__direct__') {
				...
				// System terminal — proxy through Arfanity AI backend
				const base = info.baseUrl.replace(/\/$/, '');
				authToken = token;
