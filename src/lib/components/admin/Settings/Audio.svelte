<script lang="ts">
	...
	let WAKEWORD_ENABLED = false;
	let WAKEWORD_NAME = 'Arfanity AI';
	...
	onMount(async () => {
		...
		if (res) {
			...
			WAKEWORD_ENABLED = res.tts.WAKEWORD_ENABLED || false;
			WAKEWORD_NAME = res.tts.WAKEWORD_NAME || 'Arfanity AI';
		}
...
>
	<div class=" space-y-3 overflow-y-scroll scrollbar-hidden h-full">
		<div class="flex flex-col gap-3">
			<div>
				...
				{:else if STT_ENGINE === ''}
					<div>
						...
						<div class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500">
							{$i18n.t(`Arfanity AI uses faster-whisper internally.`)}
			...
			<div>
				<div class=" mt-0.5 mb-2.5 text-base font-medium">{$i18n.t('Wakeword Detection')}</div>

				<hr class=" border-gray-100/30 dark:border-gray-850/30 my-2" />

				<div class="flex flex-col gap-2">
					<div class="flex items-center justify-between mb-2">
						<div class="text-xs font-medium">{$i18n.t('Enable Wakeword Detection')}</div>
						<label class="relative inline-flex items-center cursor-pointer">
							<input
								type="checkbox"
								bind:checked={WAKEWORD_ENABLED}
								class="sr-only peer"
							/>
							<div
								class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"
					...
					<div class="text-xs text-gray-400 dark:text-gray-500 mb-2">
						{$i18n.t(
							'Your assistant will listen for its name and activate automatically for hands-free assistance.'
						)}
					</div>

					{#if WAKEWORD_ENABLED}
						<div>
							<div class=" mb-1.5 text-xs font-medium">{$i18n.t('Wakeword Name')}</div>
							<div class="flex w-full">
								<div class="flex-1">
									<input
										class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
										bind:value={WAKEWORD_NAME}
										placeholder={$i18n.t('e.g., Arfanity AI')}
									/>
								</div>
							</div>
						</div>
				...
				<div class="mb-2">
					...
					{:else if TTS_ENGINE === 'transformers'}
						<div>
							...
							<div class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500">
								{$i18n.t(`Arfanity AI uses SpeechT5 and CMU Arctic speaker embeddings.`)}
