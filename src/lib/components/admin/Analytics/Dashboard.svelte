<script lang="ts">
	...
	const loadDashboard = async () => {
		...
		try {
			...
			if (tokensRes) {
				...
				totalCost = tokensRes.models.reduce((sum, m) => {
					const model = $models.find((mod) => mod.id === m.model_id);
					const pricing = model?.meta?.pricing;
					if (pricing) {
						return (
							sum +
							(m.input_tokens / 1000) * (pricing.input || 0) +
							(m.output_tokens / 1000) * (pricing.output || 0)
						);
					}
					return sum;
				}, 0);
	...
	const getModelCost = (modelId, tokens) => {
		...
		const pricing = model?.meta?.pricing;
		if (pricing && tokens) {
			return (tokens.input_tokens / 1000) * (pricing.input || 0) + (tokens.output_tokens / 1000) * (pricing.output || 0);
		}
		return 0;
	};
						...
						{#each sortedModels as model, idx (model.model_id)}
							...
							>
								...
								<td class="px-3 py-1 font-medium text-gray-900 dark:text-white">
									<div class="flex items-center gap-2">
										<img
											src="{AI_API_BASE_URL}/models/model/profile/image?id={model.model_id}"
											alt={model.name}
								...
								>
								<td class="px-3 py-1 text-right text-gray-900 dark:text-gray-300">
									${getModelCost(model.model_id, tokenStats[model.model_id]).toLocaleString(undefined, {
						...
						{#each sortedUsers as user, idx (user.user_id)}
							<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
								...
								<td class="px-3 py-1 font-medium text-gray-900 dark:text-white">
									<div class="flex items-center gap-2">
										<img
											src="{AI_API_BASE_URL}/users/{user.user_id}/profile/image"
											alt={user.name || 'User'}
											class="size-5 rounded-full object-cover shrink-0"
											on:error={(e) => {
