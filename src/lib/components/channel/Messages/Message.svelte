				...
				<div class="relative text-xs mb-1">
					...
					>
						{#if message?.reply_to_message?.meta?.model_id}
							<img
								src={`${AI_API_BASE_URL}/models/model/profile/image?id=${message.reply_to_message.meta.model_id}`}
						...
						{:else}
							<img
								src={message.reply_to_message.user?.role === 'webhook'
									? `${AI_API_BASE_URL}/channels/webhooks/${message.reply_to_message.user?.id}/profile/image`
									: `${AI_API_BASE_URL}/users/${message.reply_to_message.user?.id}/profile/image`}
				...
				<div class={`shrink-0 mr-1 w-9`}>
					{#if showUserProfile}
						{#if message?.meta?.model_id}
							<img
								src={`${AI_API_BASE_URL}/models/model/profile/image?id=${message.meta.model_id}`}
						...
						{:else if message.user?.role === 'webhook'}
							<ProfileImage
								src={`${AI_API_BASE_URL}/channels/webhooks/${message.user?.id}/profile/image`}
								className={'size-8 ml-0.5'}
							/>
						{:else}
							<ProfilePreview user={message.user}>
								<ProfileImage
									src={`${AI_API_BASE_URL}/users/${message.user?.id}/profile/image`}
									className={'size-8 ml-0.5'}
								/>
							</ProfilePreview>
						{/if}
