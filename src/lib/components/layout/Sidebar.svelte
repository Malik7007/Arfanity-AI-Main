			...
			<div class="-mt-[0.5px]">
				<div class="">
					<Tooltip content={$i18n.t('New Chat')} placement="right">
						<a
							class="cursor-pointer flex items-center justify-center transition-all duration-300 group {$user?.role !== 'admin' ? 'size-10 bg-blue-600 shadow-lg shadow-blue-500/30 rounded-xl text-white hover:scale-105 active:scale-95' : 'rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850'}"
							href="/"
							draggable="false"
							on:click={async (e) => {
						...
						>
							<div class="self-center flex items-center justify-center">
								<PencilSquare className="size-4.5" strokeWidth="2.5" />
							</div>
						</a>
					</Tooltip>
				</div>
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							<div class="flex items-center gap-3 w-full">
								<PencilSquare className="size-4.5" strokeWidth="2.5" />
								<span class="text-sm font-bold tracking-tight">{$i18n.t('New Chat')}</span>
								{#if $user?.role === 'admin'}
									<HotkeyHint name="newChat" className=" group-hover:visible invisible ml-auto" />
								{/if}
							</div>
						</a>
					</div>
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							<div class="flex items-center gap-3 w-full">
								<Search strokeWidth="2.5" className="size-4 text-gray-400 dark:text-gray-500" />
								<span class="text-sm font-medium text-gray-400 dark:text-gray-500">{$i18n.t('Search')}</span>
								<HotkeyHint name="search" className="group-hover:visible invisible ml-auto" />
							</div>
						</button>
					</div>
					...
					<div class=" flex-1 flex flex-col overflow-y-auto scrollbar-hidden">
						<div class="pt-1.5">
							{#if $chats}
								{#each $chats as chat, idx (`chat-${chat?.id ?? idx}`)}
									{#if idx === 0 || (idx > 0 && chat.time_range !== $chats[idx - 1].time_range)}
										<div
											class="w-full pl-2.5 font-medium {idx === 0 ? '' : 'pt-5'} pb-1.5 {$user?.role !== 'admin' ? 'date-header-advanced' : 'text-xs text-gray-500 dark:text-gray-500'}"
										>
							...
							-->
									...
									<ChatItem
										className={$user?.role !== 'admin' ? 'chat-item-advanced' : ''}
										id={chat.id}
										title={chat.title}
										createdAt={chat.created_at}
										updatedAt={chat.updated_at}
