	...
	<div slot="content">
		...
		>
			...
			{#if help}
				...
				{#if profile}
					<a 
						href={role === 'admin' ? '/static/docs/admin_guide.md' : '/static/docs/user_guide.md'}
						target="_blank" rel="noopener noreferrer" 
						draggable="false"
						class="flex rounded-xl py-1.5 px-3 w-full hover:bg-gray-100 dark:hover:bg-gray-800 transition cursor-pointer select-none"
						id="chat-help-button"
						on:click={() => {
							show = false;
						}}
					>
						...
						<div class=" self-center truncate">{$i18n.t('Help & Guide')}</div>
					</a>
				{/if}
			...
			<button
				...
				type="button"
				on:click={async () => {
					userSignOut().catch((e) => console.error(e));
					user.set(null);
