...
from arfanity_ai.models.automations import Automations, AutomationRuns, AutomationModel
from arfanity_ai.models.chats import ChatForm, Chats
from arfanity_ai.models.users import Users
from arfanity_ai.utils.task import prompt_template
from arfanity_ai.internal.db import get_async_db
...
async def _set_terminal_cwd(app, server_id: str, user, cwd: str, chat_id: str) -> None:
    ...
    Routes through the arfanity-ai terminal proxy endpoint so that
    auth headers, orchestrator policy routing, and X-User-Id are
    handled correctly — same path the frontend uses.
    """
    import aiohttp
    from arfanity_ai.env import AIOHTTP_CLIENT_SESSION_SSL
...
async def execute_automation(app, automation: AutomationModel) -> None:
    ...
    try:
        ...
        from arfanity_ai.socket.main import sio
...
async def _check_calendar_alerts(app) -> None:
    ...
    """
    from arfanity_ai.models.calendar import CalendarEvents, CalendarEventUpdateForm
    from arfanity_ai.socket.main import sio
    ...
    for event, user_tz in upcoming:
        ...
        try:
            AI_name = getattr(app.state, 'AI_NAME', 'Arfanity AI')
