...
from arfanity_ai.utils.misc import is_string_allowed
from arfanity_ai.models.oauth_sessions import OAuthSessions
from arfanity_ai.models.chats import Chats
from arfanity_ai.models.folders import Folders
from arfanity_ai.models.users import Users
from arfanity_ai.socket.main import (
    get_event_call,
    get_event_emitter,
)
from arfanity_ai.routers.tasks import (
    generate_queries,
    generate_title,
    generate_follow_ups,
...
from arfanity_ai.utils.tools import get_builtin_tools
from arfanity_ai.routers.images import (
    image_generations,
    CreateImageForm,
    image_edits,
    EditImageForm,
)
from arfanity_ai.routers.pipelines import (
    process_pipeline_inlet_filter,
    process_pipeline_outlet_filter,
)
from arfanity_ai.routers.memories import query_memory, QueryMemoryForm

from arfanity_ai.utils.webhook import post_webhook
from arfanity_ai.utils.files import (
...
from arfanity_ai.models.users import UserModel
from arfanity_ai.models.functions import Functions
from arfanity_ai.models.models import Models

from arfanity_ai.retrieval.utils import get_sources_from_items


from arfanity_ai.utils.sanitize import sanitize_code
from arfanity_ai.utils.chat import generate_chat_completion
from arfanity_ai.utils.task import (
    get_task_model_id,
    rag_template,
...
from arfanity_ai.utils.access_control import has_connection_access
from arfanity_ai.utils.plugin import load_function_module_by_id
from arfanity_ai.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)
from arfanity_ai.utils.code_interpreter import execute_code_jupyter
from arfanity_ai.utils.payload import apply_system_prompt_to_body
from arfanity_ai.utils.response import normalize_usage
from arfanity_ai.utils.mcp.client import MCPClient
...
from arfanity_ai.utils.headers import include_user_info_headers
from arfanity_ai.constants import TASKS
...
def serialize_output(output: list) -> str:
    ...
    for idx, item in enumerate(output):
        ...
        elif item_type == 'arfanity_ai:code_interpreter':
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    available_skills = []
    if all_skill_ids:
        from arfanity_ai.models.skills import Skills as SkillsModel
            ...
            if choices and choices[0].get('message', {}).get('content'):
                ...
                if content:
                    ...
                    if request.app.state.config.ENABLE_USER_WEBHOOKS and not await Users.is_user_active(user.id):
                        ...
                        if webhook_url:
                            await post_webhook(
                                request.app.state.AI_NAME,
                                webhook_url,
                                f'{content}\n\n{title} - {request.app.state.config.AI_URL}/c/{metadata["chat_id"]}',
                                {
                                    'action': 'chat',
                                    'message': content,
                                    'title': title,
                                    'url': f'{request.app.state.config.AI_URL}/c/{metadata["chat_id"]}',
                                },
                            )
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_type_map = {
                    ...
                    'code_interpreter': 'arfanity_ai:code_interpreter',
                }
                ...
                if last_type == 'message':
                    ...
                    for start_tag, end_tag in tags:
                        ...
                        if match:
                            ...
                            elif output_item_type == 'arfanity_ai:code_interpreter':
                                output.append(
                                    {
                                        'type': 'arfanity_ai:code_interpreter',
                                        'id': output_id('ci'),
                                        'status': 'in_progress',
                                        'start_tag': start_tag,
                                        'end_tag': end_tag,
                    ...
                    or (last_type == 'arfanity_ai:code_interpreter' and content_type == 'code_interpreter')
                    ...
                    elif last_type == 'arfanity_ai:code_interpreter':
            ...
            try:
                                    ...
                                    if value:
                                        ...
                                        inside_tag_block = (
                                            ...
                                            and (
                                                ...
                                                or last_item_type == 'arfanity_ai:code_interpreter'
                                                or (
                                        ...
                                        if inside_tag_block:
                                            ...
                                            if last_item_type == 'arfanity_ai:code_interpreter':
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while output and output[-1].get('type') == 'arfanity_ai:code_interpreter' and retries < MAX_RETRIES:
                        await event_emitter(
                            {
                                'type': 'chat:completion',
                                'data': {
