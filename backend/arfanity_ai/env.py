...
from arfanity_ai.constants import ERROR_MESSAGES
...
# arfanity_ai_DIR should be the directory where env.py resides (arfanity_ai/)
arfanity_ai_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of arfanity_ai_DIR (backend/)
BACKEND_DIR = arfanity_ai_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (arfanity-ai-dev/)
...
AI_NAME = os.environ.get('AI_NAME', 'Arfanity AI')
if AI_NAME != 'Arfanity AI':
    AI_NAME += ' (Arfanity AI)'

AI_FAVICON_URL = '#/favicon.png'
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('arfanity-ai')}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data('arfanity_ai', 'CHANGELOG.md') or b'').decode()
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.environ.get('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-Arfanity AI-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.environ.get('FORWARD_USER_INFO_HEADER_USER_ID', 'X-Arfanity AI-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.environ.get('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-Arfanity AI-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.environ.get('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-Arfanity AI-User-Role')
...
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.environ.get(
    'FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-Arfanity AI-Message-Id'
)
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.environ.get('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-Arfanity AI-Chat-Id')
...
AI_BUILD_HASH = os.environ.get('AI_BUILD_HASH', 'dev-build')
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', arfanity_ai_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'arfanity_ai_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', arfanity_ai_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', arfanity_ai_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', arfanity_ai_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', arfanity_ai_DIR / 'frontend')).resolve()
...
if os.path.exists(f'{DATA_DIR}/ollama.db'):
    # Rename the file
    os.rename(f'{DATA_DIR}/ollama.db', f'{DATA_DIR}/AI.db')
    log.info('Database migrated from Ollama-AI successfully.')
else:
    pass

DATABASE_URL = os.environ.get('DATABASE_URL', f'sqlite:///{DATA_DIR}/AI.db')
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f'sqlite+sqlcipher:///{DATA_DIR}/AI.db'
...
REDIS_KEY_PREFIX = os.environ.get('REDIS_KEY_PREFIX', 'arfanity-ai')
...
####################################
# AI_AUTH (Required for security)
####################################

AI_AUTH = os.environ.get('AI_AUTH', 'True').lower() == 'true'
...
AI_ADMIN_EMAIL = os.environ.get('AI_ADMIN_EMAIL', '')
AI_ADMIN_PASSWORD = os.environ.get('AI_ADMIN_PASSWORD', '')
AI_ADMIN_NAME = os.environ.get('AI_ADMIN_NAME', 'Admin')

AI_AUTH_TRUSTED_EMAIL_HEADER = os.environ.get('AI_AUTH_TRUSTED_EMAIL_HEADER', None)
AI_AUTH_TRUSTED_NAME_HEADER = os.environ.get('AI_AUTH_TRUSTED_NAME_HEADER', None)
AI_AUTH_TRUSTED_GROUPS_HEADER = os.environ.get('AI_AUTH_TRUSTED_GROUPS_HEADER', None)
AI_AUTH_TRUSTED_ROLE_HEADER = os.environ.get('AI_AUTH_TRUSTED_ROLE_HEADER', None)

# Custom header name for API key authentication.  Defaults to 'x-api-key'.
# Useful when Arfanity AI sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-Arfanity AI-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by Arfanity AI.
ENABLE_OPENAI_API_PASSTHROUGH = os.environ.get('ENABLE_OPENAI_API_PASSTHROUGH', 'False').lower() == 'true'

AI_AUTH_SIGNOUT_REDIRECT_URL = os.environ.get('AI_AUTH_SIGNOUT_REDIRECT_URL', None)

####################################
# AI_SECRET_KEY
####################################

AI_SECRET_KEY = os.environ.get(
    'AI_SECRET_KEY',
    os.environ.get('AI_JWT_SECRET_KEY', 't0p-s3cr3t'),  # DEPRECATED: remove at next major version
)

AI_SESSION_COOKIE_SAME_SITE = os.environ.get('AI_SESSION_COOKIE_SAME_SITE', 'lax')

AI_SESSION_COOKIE_SECURE = os.environ.get('AI_SESSION_COOKIE_SECURE', 'false').lower() == 'true'

AI_AUTH_COOKIE_SAME_SITE = os.environ.get('AI_AUTH_COOKIE_SAME_SITE', AI_SESSION_COOKIE_SAME_SITE)

AI_AUTH_COOKIE_SECURE = (
    os.environ.get(
        'AI_AUTH_COOKIE_SECURE',
        os.environ.get('AI_SESSION_COOKIE_SECURE', 'false'),
    ).lower()
    == 'true'
)

if AI_AUTH and AI_SECRET_KEY == '':
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.environ.get('OAUTH_CLIENT_INFO_ENCRYPTION_KEY', AI_SECRET_KEY)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.environ.get('OAUTH_SESSION_TOKEN_ENCRYPTION_KEY', AI_SECRET_KEY)
...
# Allows external apps to exchange OAuth tokens for Arfanity AI tokens
...
OTEL_SERVICE_NAME = os.environ.get('OTEL_SERVICE_NAME', 'arfanity-ai')
