...
from arfanity_ai.internal.db import Base, get_db, get_async_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(arfanity_ai_DIR / 'alembic.ini')

        # Set the script location dynamically
        migrations_path = arfanity_ai_DIR / 'migrations'
...
class AppConfig:
    ...
    def __init__(
        ...
        redis_key_prefix: str = 'arfanity-ai',
    ):
        if redis_url:
...
if JWT_EXPIRES_IN.value == '-1':
    log.warning(
        ...
        '    See: https://docs.Arfanity AI.com/reference/env-configuration\n'
    )
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', arfanity_ai_DIR / 'static')).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f'https://api.Arfanity AI.com/api/v1/custom/{CUSTOM_NAME}')
        data = r.json()
        if r.ok:
            if 'logo' in data:
                AI_FAVICON_URL = url = (
                    f'https://api.Arfanity AI.com{data["logo"]}' if data['logo'][0] == '/' else data['logo']
                )
            ...
            if 'splash' in data:
                url = f'https://api.Arfanity AI.com{data["splash"]}' if data['splash'][0] == '/' else data['splash']
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (Arfanity AI + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.arfanity-ai.svc.cluster.local:11434'
...
AI_URL = PersistentConfig('AI_URL', 'AI.url', os.environ.get('AI_URL', ''))


ENABLE_SIGNUP = PersistentConfig(
    'ENABLE_SIGNUP',
    'ui.enable_signup',
    (False if not AI_AUTH else os.environ.get('ENABLE_SIGNUP', 'True').lower() == 'true'),
)
...
try:
    banners = json.loads(os.environ.get('AI_BANNERS', '[]'))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f'Error loading AI_BANNERS: {e}')
    banners = []

AI_BANNERS = PersistentConfig('AI_BANNERS', 'ui.banners', banners)
...
MILVUS_COLLECTION_PREFIX = os.environ.get('MILVUS_COLLECTION_PREFIX', 'arfanity_ai')
...
QDRANT_COLLECTION_PREFIX = os.environ.get('QDRANT_COLLECTION_PREFIX', 'arfanity-ai')
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get('ELASTICSEARCH_INDEX_PREFIX', 'arfanity_ai_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'arfanity-ai-index')
...
# Wakeword
AUDIO_WAKEWORD_ENABLED = PersistentConfig(
    'AUDIO_WAKEWORD_ENABLED',
    'audio.wakeword.enabled',
    os.getenv('AUDIO_WAKEWORD_ENABLED', 'false').lower() == 'true',
)

AUDIO_WAKEWORD_NAME = PersistentConfig(
    'AUDIO_WAKEWORD_NAME',
    'audio.wakeword.name',
    os.getenv('AUDIO_WAKEWORD_NAME', 'Arfanity AI'),
)
