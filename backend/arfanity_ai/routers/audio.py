...
async def get_audio_config(request: Request, user=Depends(get_admin_user)):
    return {
        'tts': {
            ...
            'WAKEWORD_ENABLED': request.app.state.config.AUDIO_WAKEWORD_ENABLED,
            'WAKEWORD_NAME': request.app.state.config.AUDIO_WAKEWORD_NAME,
        },
        'stt': {
...
async def update_audio_config(request: Request, form_data: AudioConfigUpdateForm, user=Depends(get_admin_user)):
    ...
    request.app.state.config.AUDIO_WAKEWORD_ENABLED = form_data.tts.WAKEWORD_ENABLED
    request.app.state.config.AUDIO_WAKEWORD_NAME = form_data.tts.WAKEWORD_NAME
...
async def speech(request: Request, user=Depends(get_verified_user)):
    ...
    if request.app.state.config.TTS_ENGINE == 'openai':
        ...
        try:
            ...
            status_code = 500
            detail = f'Arfanity AI: Server Connection Error'
...
def transcription_handler(request, file_path, metadata, user=None):
    ...
    if request.app.state.config.STT_ENGINE == '':
        ...
        try:
            ...
            raise Exception(detail if detail else 'Arfanity AI: Server Connection Error')
