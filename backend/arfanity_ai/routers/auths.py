...
from arfanity_ai.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    Token,
...
from arfanity_ai.models.users import (
    UserModel,
...
from arfanity_ai.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
from arfanity_ai.env import (
    AI_AUTH,
    AI_AUTH_TRUSTED_EMAIL_HEADER,
    AI_AUTH_TRUSTED_NAME_HEADER,
    AI_AUTH_TRUSTED_GROUPS_HEADER,
    AI_AUTH_TRUSTED_ROLE_HEADER,
    AI_AUTH_COOKIE_SAME_SITE,
...
from arfanity_ai.utils.oauth import auth_manager_config
from pydantic import BaseModel

from arfanity_ai.utils.misc import parse_duration, validate_email_format
...
from arfanity_ai.internal.db import get_async_session
from sqlalchemy.ext.asyncio import AsyncSession
from arfanity_ai.utils.webhook import post_webhook
from arfanity_ai.utils.access_control import get_permissions, has_permission
from arfanity_ai.utils.groups import apply_default_group_assignment

from arfanity_ai.utils.redis import get_redis_client
from arfanity_ai.utils.rate_limit import RateLimiter
...
async def create_session_response(
    ...
    if set_cookie and response:
        ...
        response.set_cookie(
            ...
            httponly=True,
            samesite=AI_AUTH_COOKIE_SAME_SITE,
            secure=AI_AUTH_COOKIE_SECURE,
...
async def update_password(
    ...
    if AI_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(
    ...
    if AI_AUTH_TRUSTED_EMAIL_HEADER:
        if AI_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[AI_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if AI_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(AI_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = await Auths.authenticate_user_by_email(email, db=db)
        if user:
            if AI_AUTH_TRUSTED_GROUPS_HEADER:
                group_names = request.headers.get(AI_AUTH_TRUSTED_GROUPS_HEADER, '').split(',')
                group_names = [name.strip() for name in group_names if name.strip()]

                if group_names:
                    await Groups.sync_groups_by_group_names(user.id, group_names, db=db)

            if AI_AUTH_TRUSTED_ROLE_HEADER:
                trusted_role = request.headers.get(AI_AUTH_TRUSTED_ROLE_HEADER, '').lower().strip()
...
async def signout(request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f'&post_logout_redirect_uri={AI_AUTH_SIGNOUT_REDIRECT_URL}'
                                            if AI_AUTH_SIGNOUT_REDIRECT_URL
                                            else ''
                                        ),
                                    },
                                    headers=response.headers,
    ...
    if AI_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                'status': True,
                'redirect_url': AI_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        'ADMIN_EMAIL': request.app.state.config.ADMIN_EMAIL,
        'AI_URL': request.app.state.config.AI_URL,
...
async def update_admin_config(request: Request, form_data: AdminConfig, user=Depends(get_admin_user)):
    ...
    request.app.state.config.AI_URL = form_data.AI_URL
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an Arfanity AI JWT.
