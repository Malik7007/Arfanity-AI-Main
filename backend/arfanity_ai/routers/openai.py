...
from arfanity_ai.utils.access_control import has_connection_access, check_model_access
...
from arfanity_ai.utils.payload import (
...
from arfanity_ai.utils.session_pool import (
    cleanup_response,
    get_session,
    stream_wrapper,
)
...
from arfanity_ai.utils.anthropic import is_anthropic_url, get_anthropic_models
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'Arfanity AI: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='Arfanity AI: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    Arfanity AI stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='Arfanity AI: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
