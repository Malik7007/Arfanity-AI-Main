...
from arfanity_ai.models.channels import (
    Channels,
    ChannelModel,
    ChannelForm,
    ChannelResponse,
...
from arfanity_ai.models.access_grants import AccessGrants, has_public_read_access_grant, has_public_write_access_grant
from arfanity_ai.models.messages import (
    Messages,
    MessageModel,
    MessageResponse,
...
from arfanity_ai.utils.files import get_image_base64_from_file_id

from arfanity_ai.config import ENABLE_ADMIN_CHAT_ACCESS, ENABLE_ADMIN_EXPORT
from arfanity_ai.constants import ERROR_MESSAGES
from arfanity_ai.env import STATIC_DIR
...
from arfanity_ai.utils.access_control import has_permission, filter_allowed_access_grants
from arfanity_ai.utils.webhook import post_webhook
from arfanity_ai.utils.channels import extract_mentions, replace_mentions
...
async def send_notification(request, channel, message, active_user_ids, db=None):
    name = request.app.state.AI_NAME
            ...
            if enable_user_webhooks and u.settings:
                ...
                if webhook_url:
                    await post_webhook(
                        name,
                        webhook_url,
                        f'#{channel.name} - {AI_url}/channels/{channel.id}\n\n{message.content}',
                        {
                            'action': 'channel',
                            'message': message.content,
                            'title': channel.name,
                            'url': f'{AI_url}/channels/{channel.id}',
                        },
                    )
