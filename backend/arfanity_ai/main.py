...
from arfanity_ai.utils import logger
from arfanity_ai.utils.asgi_middleware import (
    AuthTokenMiddleware,
    CommitSessionMiddleware,
    RedirectMiddleware,
    WebsocketUpgradeGuardMiddleware,
)
from arfanity_ai.utils.audit import AuditLevel, AuditLoggingMiddleware
from arfanity_ai.utils.logger import start_logger
from arfanity_ai.utils.session_pool import get_session
from arfanity_ai.socket.main import (
    MODELS,
    app as socket_app,
...
from arfanity_ai.routers import (
    analytics,
    audio,
    images,
    ollama,
...
from arfanity_ai.routers.retrieval import (
...
from arfanity_ai.internal.db import ScopedSession, engine, get_async_session

from arfanity_ai.models.functions import Functions
from arfanity_ai.models.models import Models
from arfanity_ai.models.users import UserModel, Users
from arfanity_ai.models.chats import Chats, ChatForm

from arfanity_ai.config import (
    # Ollama
    ENABLE_OLLAMA_API,
    OLLAMA_BASE_URLS,
    OLLAMA_API_CONFIGS,
...
from arfanity_ai.env import (
    ...
    AI_BUILD_HASH,
    AI_SECRET_KEY,
    AI_SESSION_COOKIE_SAME_SITE,
    AI_SESSION_COOKIE_SECURE,
    ENABLE_SIGNUP_PASSWORD_CONFIRMATION,
    AI_AUTH_TRUSTED_EMAIL_HEADER,
    AI_AUTH_TRUSTED_NAME_HEADER,
    AI_AUTH_SIGNOUT_REDIRECT_URL,
    # SCIM
    ENABLE_SCIM,
    SCIM_TOKEN,
...
from arfanity_ai.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
    get_filtered_models,
)
from arfanity_ai.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
)
from arfanity_ai.utils.actions import chat_action as chat_action_handler
from arfanity_ai.utils.embeddings import generate_embeddings
from arfanity_ai.utils.middleware import (
    build_chat_response_context,
    process_chat_payload,
    process_chat_response,
)
from arfanity_ai.utils.tools import set_tool_servers, set_terminal_servers

from arfanity_ai.utils.auth import (
    get_license_data,
...
from arfanity_ai.utils.plugin import install_tool_and_function_dependencies
from arfanity_ai.utils.oauth import (
...
from arfanity_ai.utils.security_headers import SecurityHeadersMiddleware
from arfanity_ai.utils.redis import get_redis_connection

from arfanity_ai.tasks import (
...
from arfanity_ai.utils.redis import get_sentinels_from_env


from arfanity_ai.constants import ERROR_MESSAGES, TASKS
...
Developing the best AI user interface.
{f"Commit: {AI_BUILD_HASH}" if AI_BUILD_HASH != "dev-build" else ""}
https://github.com/Malik7007
"""
)
...
async def lifespan(app: FastAPI):
    ...
    if AI_ADMIN_EMAIL and AI_ADMIN_PASSWORD:
        if await create_admin_user(AI_ADMIN_EMAIL, AI_ADMIN_PASSWORD, AI_ADMIN_NAME):
    ...
    from arfanity_ai.utils.automations import scheduler_worker_loop
    ...
    from arfanity_ai.utils.session_pool import close_session
...
if ENABLE_OTEL:
    from arfanity_ai.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.AI_URL = AI_URL
...
app.state.config.BANNERS = AI_BANNERS
...
from arfanity_ai.utils.access_control import migrate_access_control
...
app.state.AUTH_TRUSTED_EMAIL_HEADER = AI_AUTH_TRUSTED_EMAIL_HEADER
app.state.AUTH_TRUSTED_NAME_HEADER = AI_AUTH_TRUSTED_NAME_HEADER
app.state.AI_AUTH_SIGNOUT_REDIRECT_URL = AI_AUTH_SIGNOUT_REDIRECT_URL
...
# `arfanity_ai.utils.asgi_middleware` for the rationale.
...
from arfanity_ai.utils.anthropic import (
...
async def generate_messages(
    ...
    All models configured in Arfanity AI are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/arfanity-ai/arfanity-ai/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Arfanity AI.
...
try:
    if ENABLE_STAR_SESSIONS_MIDDLEWARE:
        ...
        app.add_middleware(
            ...
            cookie_same_site=AI_SESSION_COOKIE_SAME_SITE,
            cookie_https_only=AI_SESSION_COOKIE_SECURE,
        )
    ...
    app.add_middleware(
        ...
        same_site=AI_SESSION_COOKIE_SAME_SITE,
        https_only=AI_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            'name': app.state.AI_NAME,
            'short_name': app.state.AI_NAME,
            'description': f'{app.state.AI_NAME} is an open, extensible, user-friendly interface for AI that adapts to your workflow.',
            'start_url': '/',
            'display': 'standalone',
...
async def get_opensearch_xml():
    ...
    <ShortName>{app.state.AI_NAME}</ShortName>
    <Description>Search {app.state.AI_NAME}</Description>
    <InputEncoding>UTF-8</InputEncoding>
    <Image width="16" height="16" type="image/x-icon">{app.state.config.AI_URL}/static/favicon.png</Image>
    <Url type="text/html" method="get" template="{app.state.config.AI_URL}/?q={'{searchTerms}'}"/>
    <moz:SearchForm>{app.state.config.AI_URL}</moz:SearchForm>
